...
export async function POST(req: NextRequest) {
  try {
    ...
    const { to, message, template, templateParams, templateComponents, templateLang, mediaUrl, mediaType } = body;
    ...
    if (template) {
      ...
      if (mediaType === "image") {
        ...
        waBody = {
          ...
          to,
          type: "document",
          document: { link: fullUrl },
        };
        savedMessage = message || "";
      } else {
        return NextResponse.json({ error: "mediaType must be image, video, or document" }, { status: 400 });
      }
    } else {
      // Send text message
