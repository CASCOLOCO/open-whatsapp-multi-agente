...
export function PanelSidebar({ userName }: { userName: string }) {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-[60px] min-h-screen bg-[#1e2a3a] flex-col items-center shrink-0 z-50 border-r border-[#2a3a4a]">
        {/* Logo top */}
        <div className="py-4">
          <AppLogo size={36} />
        </div>
        ...
        <nav className="flex-1 flex flex-col items-center gap-0.5 py-2 w-full overflow-y-auto">
          ...
          {/* Divider */}
          <div className="w-8 h-px bg-[#2a3a4a] my-2" />
          ...
          <div className="relative w-full flex justify-center">
            <button
              ...
              className={`w-[44px] h-[44px] flex items-center justify-center rounded-xl transition-all duration-150 ${
                isWhatsApp
                  ? "bg-[#25d366]/20 text-[#25d366]"
                  : "text-[#8899aa] hover:text-[#25d366] hover:bg-[#2a3a4a]"
              }`}
              onMouseEnter={() => showTooltip("whatsapp")}
              onMouseLeave={hideTooltip}
            >
              <img src="/logo-sm.png" alt="CRM" className="w-7 h-7 rounded-full object-cover" />
            </button>
        ...
        {/* Bottom icons */}
        <div className="flex flex-col items-center gap-1 py-3 border-t border-[#2a3a4a] w-full">
          <button
            onClick={() => { setSearchOpen(true); setTimeout(() => searchRef.current?.focus(), 100); }}
            className="w-[44px] h-[44px] flex items-center justify-center rounded-xl text-[#8899aa] hover:text-white hover:bg-[#2a3a4a] transition-all"
          ...
          <button
            ...
            className="w-[44px] h-[44px] flex items-center justify-center rounded-xl text-[#8899aa] hover:text-red-400 hover:bg-[#2a3a4a] transition-all"
      ...
      <div
        className={`hidden md:flex min-h-screen bg-[#1a2535] border-r border-[#2a3a4a] flex-col shrink-0 transition-all duration-200 overflow-hidden z-40 ${
          waExpanded ? "w-[200px] opacity-100" : "w-0 opacity-0"
        }`}
      >
        <div className="px-4 py-4 border-b border-[#2a3a4a]">
          <h2 className="text-white font-semibold text-sm flex items-center gap-2">
            <img src="/logo-sm.png" alt="CRM" className="w-6 h-6 rounded-full object-cover" />
            WhatsApp CRM
          </h2>
        </div>
          ...
          {whatsappSubItems.map((item) => {
            ...
            if ((item as any).action) {
              return (
                <button
                  key={item.href}
                  onClick={() => window.dispatchEvent(new CustomEvent((item as any).action))}
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-all text-[#8899aa] hover:text-white hover:bg-[#2a3a4a]/50"
                >
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-all ${
                  active
                    ...
                    : "text-[#8899aa] hover:text-white hover:bg-[#2a3a4a]/50"
                }`}
              >
                <span className={active ? "text-[#25d366]" : "text-[#667788]"}>
                  {(icons as any)[item.icon]}
                </span>
                <span>{item.label}</span>
              </Link>
